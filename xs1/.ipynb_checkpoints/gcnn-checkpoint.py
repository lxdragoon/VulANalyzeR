...
from tensorflow.keras.layers import (GRU, LSTM, Bidirectional, Dense, Dropout,
                                     Embedding, Input, LayerNormalization, Flatten, GlobalMaxPooling1D)
from tensorflow.keras.optimizers import SGD, Adam
from tensorflow.keras.preprocessing.text import Tokenizer
from tf2_gnn.layers import (GNN, GNNInput)
from xs1.nodes2graph import NodesToGraphRepresentationInput, WeightedSumGraphRepresentation

from xs1.dataset import Sample, gen_juliet, gen_ndss, gen_esh,  Dataset
...
def run(ds: Tuple[Dataset, Dataset, Dataset], ds_name,
    ...
    print('print ratio of classes: ', sum(train.gob)/len(train.gob))
...
def build(num_cwe):
    ...
    #gru1 = Bidirectional(GRU(100, return_sequences=True))(embedding)
    #gru2 = Bidirectional(GRU(100))(gru1)

    flat_node_fea = GlobalMaxPooling1D()(embedding)
    node_fea = Dense(100)(flat_node_fea)

    flat_adj = tf.reshape(adj_in, ([-1, 2]))

    gnn_input = GNNInput(
        node_features=node_fea,
    ...
    wsgr, attn = WeightedSumGraphRepresentation(
        16, 16, weighting_fun='sigmoid', name='weightedSum')(reducer_input)

    reduced = tf.reshape(wsgr, [graphs, 16])
    ...
    md_infer = tf.keras.Model(
        inputs=(seq_in, n2g_in, adj_in), outputs=[cwe, gob, attn])
...
def run_test(ds: Tuple[Dataset, Dataset, Dataset], test_ds: Dataset, ds_name, ds_name_test,
    ...
    def eval_save():
        ...
        to_csv = []
        for sample, cwe, gob, pred_cwe, pred_gob, attn in zip(
                test_ds.x, test_ds.cwe.tolist(), test_ds.gob.tolist(),
                predictions[0].tolist(), predictions[1].tolist(),
                predictions[2].to_list()):
            to_csv.append([
                sample.file_name,
...
if __name__ == "__main__":
    #run(gen_ndss(), 'ndss', batch_size=96, epochs=30, train_op=True, saving_model = True)
    run(gen_juliet(), 'juliet', batch_size=64, epochs=10, train_op=True, saving_model = True)
    #run_test(gen_juliet(), gen_esh('juliet'), 'juliet', 'esh', batch_size = 16, epochs = 1, train_op = False, saving_model = False)
    #run_test(gen_ndss(), gen_esh('ndss'), 'ndss', 'esh', batch_size = 16, epochs = 1, train_op = False, saving_model = False)
