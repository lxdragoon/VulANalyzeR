...
from xs1.tokenizers.tokenizer import blk2seq
...
def json2sample_devign(js_file, fn_label=None) -> Sample:
    ...
    obj = AttrDict.from_nested_dict(obj)
    target_func = str(js_file).split('.')[2]
...
def gen(dir, fn_label, test=0.2, save=True) -> Tuple[Dataset, Dataset, Dataset]:
    ...
    all_fs = list(Path(dir).glob('**/*.asm.json')) if not 'devign' in dir else list(Path(dir).glob('**/*.json.gz'))
