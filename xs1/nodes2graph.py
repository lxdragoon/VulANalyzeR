...
class WeightedSumGraphRepresentation(NodesToGraphRepresentation):
    ...
    def get_config(self):
        ...
        config.update({
            'graph_representation_size': self._graph_representation_size,
            'num_heads':  self._num_heads,
            'weighting_fun':  self._weighting_fun,
            'scoring_mlp_layers':  self._scoring_mlp_layers,
            'scoring_mlp_activation_fun':  self._scoring_mlp_activation_fun,
            'scoring_mlp_dropout_rate':  self._scoring_mlp_dropout_rate,
            'transformation_mlp_layers':  self._transformation_mlp_layers,
            'transformation_mlp_activation_fun':  self._transformation_mlp_activation_fun,
            'transformation_mlp_dropout_rate':  self._transformation_mlp_dropout_rate,
        })
        return config
    ...
    def build(self, input_shapes: NodesToGraphRepresentationInput):
        with tf.name_scope("WeightedSumGraphRepresentation"):
            if self._weighting_fun not in ("none", "average"):
                ...
                self._scoring_mlp.build(tf.TensorShape(
                    (None, input_shapes.node_embeddings[-1])))
            ...
            self._transformation_mlp.build(tf.TensorShape(
                (None, input_shapes.node_embeddings[-1])))
    ...
    def call(self, inputs: NodesToGraphRepresentationInput, training: bool = False):
        ...
        if self._weighting_fun not in ("none", "average"):
            scores = self._scoring_mlp(
                inputs.node_embeddings, training=training)  # Shape [V, H]
        ...
        if self._weighting_fun == "none":
            ...
            weighted_node_reprs = tf.reshape(
                weighted_node_reprs, shape=(-1,
                                            self._graph_representation_size)
            )  # Shape [V, GD]
            graph_reprs = tf.math.segment_sum(
                data=weighted_node_reprs, segment_ids=inputs.node_to_graph_map
            )  # Shape [G, GD]

        return graph_reprs, weights
